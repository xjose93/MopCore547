    ...
    _navMeshQuery(NULL)
{
    ...
    sLog->outDebug(LOG_FILTER_MAPS, "++ PathGenerator::PathGenerator for %s (GUID: %u Entry: %u)", _sourceUnit->GetName(), _sourceUnit->GetGUID(), _sourceUnit->GetEntry());
...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ, bool forceDest, bool straightLine)
{
    ...
    if (!MoPCore::IsValidMapCoord(destX, destY, destZ) || !MoPCore::IsValidMapCoord(x, y, z))
        return false;
    ...
    sLog->outDebug(LOG_FILTER_MAPS, "++ PathGenerator::CalculatePath() for %s (GUID: %u Entry: %u)", _sourceUnit->GetName(), _sourceUnit->GetGUID(), _sourceUnit->GetEntry());
...
void PathGenerator::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
    ...
    else if (startPolyFound && !endPolyFound)
    {
        ...
        if (!suffixPolyLength || dtStatusFailed(dtResult))
        {
            ...
            sLog->outError(LOG_FILTER_MAPS, "%s's (GUID: %u Entry : %u) Path Build failed: 0 length path", _sourceUnit->GetName(), _sourceUnit->GetGUID(), _sourceUnit->GetEntry());
        }
    ...
    else
    {
        ...
        if (!_polyLength || dtStatusFailed(dtResult))
        {
            ...
            sLog->outError(LOG_FILTER_MAPS, "%s's (GUID: %u Entry : %u) Path Build failed: 0 length path", _sourceUnit->GetGUID(), _sourceUnit->GetEntry());
            BuildShortcut();
            _type = PATHFIND_NOPATH;
            return;
        }
